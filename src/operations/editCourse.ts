...
export async function editCourse({ courseId, name, courseImg, type, professorName, professorImg, local }: courseProps) {
  try {
    ...
    const downloadCourseURL = courseImg !== "generic" ? await getDownloadURL(courseSnapshot.ref) : undefined;
    const downloadProfessorURL = professorImg !== "generic" ? await getDownloadURL(professorSnapshot.ref) : undefined;
